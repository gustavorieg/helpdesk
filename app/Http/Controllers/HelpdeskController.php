...
class HelpdeskController extends Controller
{
    ...
    public function ticketStore(TicketFormRequest $request){
        ...
        $ticket = new Ticket;
        $ticket->requisitante = Auth::user()->name;
        $ticket->requisitanteId = Auth::user()->id;
    ...
    public function ticketUpdate(Request $request){
        ...
        DB::table('tickets')->where('id', $request->id)->update(['atribuido' => $request->atribuido]);
        DB::table('tickets')->where('id', $request->id)->update(['status' => 'em progresso']);
        return redirect('/tickets');
   }
}
